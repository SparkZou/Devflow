...
左栏按「客户项目 / 自己的项目」列出所有项目（同一产品的前后端合并成一项），每项后面是 **需要你处理 / 自动进行中** 的数量；
点进去右侧只看这个项目：GitHub / PR / Issues / 部署记录链接、线上监控、收件框（项目已预选）和任务三栏。「全部」是总览，「未归类」是还没选项目的任务。

线上监控每 10 分钟自动检查每个项目的 `deploy_url`：健康状态（HTTP 状态码、响应时间）和 **HTTPS 证书到期时间**。
证书剩 30 天内左栏出现 🔒 标记，14 天内标红并写进每天的日报（`devflow digest` 也会列出）；「立即检查」可手动刷新。

### 手机上用（主要入口）

面板是响应式的：手机上项目导航变成顶部可横滑的胶囊条，收件框在最前面。建议在 Safari / Chrome 里「添加到主屏幕」，之后像 App 一样打开（有图标、全屏、登录态保持 7 天）。

微信里拿到需求 → 长按消息「复制」→ 打开 DevFlow → 长按输入框「粘贴」→ 选项目（可不选）→ 「交给 AI」。截图：微信里长按图片「复制」后同样粘贴，或先保存到相册再点「从相册选择」（可多张）。合并 PR、发送客户等确认操作在手机上一样点按钮即可。
